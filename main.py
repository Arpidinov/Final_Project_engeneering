...
class Background(Widget):
    ...
    def scroll_textures(self, time_passed):
        ...
        self.floor_texture.uvpos = ( (self.floor_texture.uvpos[0] + time_passed/6.0)%Window.width, self.floor_texture.uvpos[1])
...
class MainApp(App):
    ...
    def start_game(self):
        ...
        for i in range(num_pipes):
            pipe = Pipe()
            pipe.pipe_center = randint(200 , self.root.height -200)
            pipe.size_hint = (None, None)
            pipe.pos = (Window.width + i*distance_between_pipes, 85)
            pipe.size = (64, self.root.height - 85)
